...
export async function streamLangChainResponse({
  ...
  console.log("🔍 [streamLangChainResponse] 开始处理结果:", { 
    resultType: typeof result, 
    result: result,
    hasActionExecution: !!actionExecution,
    actionExecution 
  });
  ...
  if (typeof result === "string") {
    console.log("📝 [streamLangChainResponse] 处理字符串结果:", result);
    if (!actionExecution) {
      // Just send one chunk with the string as the content.
      console.log("💬 [streamLangChainResponse] 发送文本消息");
      eventStream$.sendTextMessage(randomId(), result);
    } else {
      // Send as a result
      console.log("📤 [streamLangChainResponse] 发送 Action 执行结果:", {
        actionExecutionId: actionExecution.id,
        actionName: actionExecution.name,
        result: result,
      });
